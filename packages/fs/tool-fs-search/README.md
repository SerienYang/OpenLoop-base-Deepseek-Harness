...
The glob description states the configured over-cap ordering. The generated `glob` and `grep` schemas use `sampleOverCapGlobResults: true`; the tools are registered unconditionally.
