...
`credentialMode` is optional and defaults to `api-key`, preserving the provider-native/request `apiKey` behavior above. `bearer` requires `apiKeyEnv` and sends that resolved named credential only as `Authorization: Bearer …`; it does not pass a request `apiKey`, does not consult catalog ambient authentication, and suppresses configured `Authorization`, `x-api-key`, and `api-key` headers case-insensitively so they cannot collide. Bearer mode is valid only when every model on the route uses one of the generic protocols returned by `supportedProtocols()`.
...
Credentials never enter that collection. The harness resolves a route's key through its own seam before the request reaches pi-ai. In `api-key` mode it passes the key as the request's `apiKey` option, which pi-ai treats as the highest-priority provider-native auth override. In `bearer` mode it does not pass a request `apiKey`; instead, the adapter writes exactly `Authorization: Bearer <key>` and removes configured `Authorization`, `x-api-key`, and `api-key` headers before dispatch. `Models` therefore holds no credential store, and the harness keeps its fail-loud reference semantics. An `api-key` route naming no credential resolves as configured-but-keyless and leaves the requirement to the protocol, which is where it actually lives; bearer routes must name `apiKeyEnv`.
