...
| `credentialHeaders` | http | 否 | 将 header 名映射到 `{ ref, prefix? }`；每次请求对每个不同 reference 解析一次 |
...
- Streamable HTTP 凭据 header 会在每次请求时通过 `ctx.credentials` 解析；没有该服务时读取启动环境。每个不同 reference 只解析一次形成请求级快照，并复用于所有对应 header。已配置 header、输入 `Request` header 和 `init.headers` 按此顺序合并，credential header 最后写入；解析过程响应最终生效的请求取消信号。网络发送前会校验 header 名称、前缀和解析值；凭据 header 不能与字面量 header 或 MCP 协议保留 header 冲突。带凭据的请求会在跟随跳转前拒绝 redirect，其他非成功响应则会在 SDK 读取前缩减为仅含状态码的响应，避免服务器回显的认证信息进入错误或日志。
