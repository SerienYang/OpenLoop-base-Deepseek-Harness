...
- Streamable HTTP 凭据 header 会在每次请求时通过 `ctx.credentials` 解析；没有该服务时读取启动环境。网络发送前会校验 header 名称、前缀和解析值；凭据 header 不能与字面量 header 或 MCP 协议保留 header 冲突。带凭据的请求会在跟随跳转前拒绝 redirect，避免认证信息跨 origin 传递。同一服务器可在多个 header 中使用同一个 credential reference。
