...
- Streamable HTTP credential headers resolve through `ctx.credentials` for every request (or the launch environment when that service is absent). Header names, prefixes, and resolved values are validated before dispatch; credential headers cannot collide with literal or MCP protocol-owned headers. One server may use one credential reference across multiple headers.
