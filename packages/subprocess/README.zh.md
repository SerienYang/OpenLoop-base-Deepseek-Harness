...
子系统参考——spawn spec、输出读取器、结果、`DSH_*` 环境——见 docs/subsystems/subprocess.md；seam 决定见 subprocess seam Agent Note。
