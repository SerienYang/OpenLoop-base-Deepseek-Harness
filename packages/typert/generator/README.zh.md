...
包根导出中包含本仓库 Cordis 运行时目录使用的模型驱动提取逻辑、完整性检查和确定性文本渲染器。它们接受 `CordisCatalogPolicy`；由仓库持有的运行时类型分类和继承 Cordis 条目位于 `scripts/cordis-inspect-policy.ts`，并由调用方显式传入。因此，生成器包只包含投影机制，不会隐式复制仓库策略。
