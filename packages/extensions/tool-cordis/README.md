...
`cordis_inspect what:"api"` / `what:"events"` renders `src/api-catalog.ts`, the generated projection of the workspace's Cordis declarations: rendered method signatures, source JSDoc, harness events with their dispatch modes, and the type shapes those signatures reference. It is a compile-time fact about the repository, so `pnpm run gen-cordis-inspect-catalog` regenerates the Host and Client catalogs together and `pnpm run verify-cordis-inspect-catalog` gates their freshness.
...
Namespace plugin: named exports `name` / `inject` / `apply`, no default export (docs/postmortem/0001). It injects `tools` and `dynamicCordisRunner`.
...
The conversation model sees the generated `cordis_inspect`, `cordis_define`, `cordis_run`, `cordis_stop`, and `cordis_undefine` schemas whenever this plugin is visible.
