...
可选的版本 1 `ctx.browserApiPolicy` 服务存在时，Gateway 会在 descriptor 查找、
provider 解析、receiver 访问或 `Reflect.apply` 之前检查精确的
`<namespace>/<method>` endpoint。policy 挂载期间，所有语法合法的 Typert
endpoint（包括被拒绝及未知 endpoint）都会被 Gateway 认领，因此 API Proxy
fallback 无法绕过拒绝。required-inject policy 的 Gateway 在 provider 卸载期间
仍会认领并拒绝请求。默认 DSH 不挂载 policy，并保留既有分派行为。
...
- Connection 适配器将普通分发故障和业务异常映射为 RPC 的 `internal` 代码，且不附带详细信息；浏览器 policy 拒绝保留 `policy-denied`，`TypertLookupFailure` 携带的 lookup 策略错误会原样返回。其他结构化的 `TypertGatewayError` 类别仅供同进程调用方使用。
